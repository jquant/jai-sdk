...
from jai.core.utils_funcs import data2json, df2json, resolve_db_type, series2json
...
@pytest.mark.parametrize(
    "col1, col2, ids",
    [([42, 123], ["abc", "def"], None), ([69, 420], ["ghi", "jkl"], [10, 64])],
)
def test_df2json(col1, col2, ids):
    ...
    if ids is None:
        ids = range(len(col1))
    out = ",".join(
        [f'{{"id":{i},"col1":{a},"col2":"{b}"}}' for i, a, b in zip(ids, col1, col2)]
    )
    assert df2json(df) == "[" + out + "]", "df2json failed."


@pytest.mark.parametrize(
    "col_name, db_type", [("text", "Text"), ("image_base64", "Image")]
)
...
def test_data2json(setup_dataframe, setup_img_data, dtype, col_name, db_type):
    ...
    if dtype == "df":
        data = data.to_frame()
        ids = data.index
        s = pd.Series(data[col_name], index=pd.Index(ids, name="id"), name=col_name)
        gab = s.reset_index().to_json(orient="records")
    elif dtype == "df_id":
...
@pytest.mark.parametrize(
    "col1, col2, ids",
    [([42, 123], ["abc", "def"], [2, 2]), ([69, 420], ["ghi", "jkl"], [42, 42])],
)
...
def test_read_image_corrupted_ignore(folder, handle_errors):
    ...
    data = read_image(
        folder=folder, id_pattern="img(\d+)_corrupted", handle_errors=handle_errors
    )
...
def test_check_dtype_and_clean():
    ...
    r = 1100
    data = pd.DataFrame(
        {"category": [str(i) for i in range(r)], "number": [i for i in range(r)]}
    )
...
@pytest.mark.parametrize(
    "db_type, col, ans",
    [
        ({"col1": "FastText"}, "col1", "FastText"),
        ({"col1": "FastText"}, "col2", "TextEdit"),
    ],
)
