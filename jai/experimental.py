...
class Name():
    ...
    def _analyze(self):
        ...
        is_fill = (self.data.isna().sum() > 0) & (self._analysis.loc['unique']/self._analysis.loc['count'] < .05)
    ...
    def fit(self, name=None, overwrite=False):
        ...
        for col in self.columns[self._process.loc['resolution']]:
            ...
            name = 'r' + self.gen_name + col.lower().replace("-", "_").replace(" ", "_")
            self.__jai.embedding(name, self.data[col], overwrite=overwrite)

        for col in self.columns[self._process.loc['fill']]:
            print(f"Fill {col}")
            name = 'f' + self.gen_name + col.lower().replace("-", "_").replace(" ", "_")
            self.__jai.fill(name, self.data, col)

        print("Sanity")
        name = 's' + self.gen_name
        self.__jai.sanity(name, self.data, columns_ref=self.columns[self._process.loc['sanity']])
    ...
    def solve(self, task, column):
        ...
        if task == "resolution":
            name = 'r' + self.gen_name + column.lower().replace("-", "_").replace(" ", "_")
            results = self.__jai.resolution(name, self.data[column])
            return process_similar(results, return_self=True)

        elif task == "fill":
            name = 'f' + self.gen_name + column.lower().replace("-", "_").replace(" ", "_")
            results = self.__jai.fill(name, self.data, column)
            return process_predict(results)

        elif task == "sanity":
            name = 's' + self.gen_name
            results = self.__jai.sanity(name, self.data, columns_ref=self.columns[self._process.loc['sanity']])
            return process_predict(results)
        else:
